...
g_o_m{(phase-1)*ngps+1}.init_time=GetSecs();
qz=clock;g_o_m{(phase-1)*ngps+1}.init_time_r=qz;
PsychDefaultSetup(2);bgColor=[128 128 128];
Screen('Preference','SkipSyncTests',1);%skip sync tests
devices=PsychHID('Devices');
if inpo==1;for i=1:length(devices);if strcmpi(devices(i).usageName,'Mouse');jsnum=i;end;end
    %jsnum=2;%2=laptop,1=non-scanning otherwise
...
for ng=1:pngs
    if phase<=scpha;gtrial=(phase-1)*ngps+ng;g_id=g_o(gtrial);ml_id=ml(g_id);
    elseif phase<=scpha*2;gtrial=(phase-1)*ngps+ng;g_id=g_o(gtrial-tngs);ml_id=ml(g_id);
    ...
    if phase<=scpha
        ...
        WaitSecs(3*TR);
        g_o_m{gtrial}.timelog(tl)=GetSecs();tl=tl+1;
        %g_o_m=playMovEZ(g_o_m,fn,fmri,window,root,ml_id-5,mode,sub,phase,gtrial,qz,et,suspy);Screen('Close', window);%run short
        g_o_m=playMovEZ(g_o_m,fn,fmri,window,root,0,mode,sub,phase,gtrial,qz,et,suspy);Screen('Close', window);
        %preference test
        g_o_m{gtrial}.timelog(tl)=GetSecs();tl=tl+1;
        [window, windowRect] = PsychImaging('OpenWindow', screenNumber, white); %open "on screen
        Screen('BlendFunction', window, 'GL_SRC_ALPHA', 'GL_ONE_MINUS_SRC_ALPHA');%alpha blend for smoothness
        ...
        Screen('Flip',window);
        g_o_m{gtrial}.timelog(tl)=GetSecs();tl=tl+1;
        inside=zeros(size(eBoxes,2),1);answer = 0;eBox=0;
        if inpo==1
            while sum(buttons) == 0
                [mx, my, buttons] = GetMouse(window);
