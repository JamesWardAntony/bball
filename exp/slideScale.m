...
elseif strcmp(device, 'joystick')% jwa
    responseKey   = 1; % first joystick button
end
...
while answer == 0
    ...
    elseif strcmp(device, 'joystick')% jwa
        axisState = [Gamepad('GetAxis', 1, 1), Gamepad('GetAxis', 1, 2)];
        x =  [(axisState(1)/33000)*(rect(3)/2) + center(1)];
    else
    ...
    % DrawFormattedText(screenPointer, question, 'center',rect(4)*(scalaPosition - 0.1)); % jwa
    DrawFormattedText(screenPointer, question, 'center','center'); 
    ...
    elseif strcmp(device, 'joystick')% jwa
        [keyIsDown, secs, keyCode] = PsychHID('KbCheck', 3);
        if keyCode(responseKey) == 1
            answer = 1;
        end
    else
